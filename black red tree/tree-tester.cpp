...
void del(Node *deleted) {
    ...
    if ((deleted->color == black) && (deleted->left == nullptr) && (deleted->right == nullptr)) {
        ...
        } else if ((deleted->parent->color == black) && (bro(deleted)->color == black) &&
                   (bro(deleted)->left->color == black) && (bro(deleted)->right->color == black)) {
            bro(deleted)->color = red;
        } else if ((deleted->parent->color == red) && (bro(deleted)->color == black) &&
                   (bro(deleted)->left->color == black) && (bro(deleted)->right->color == black)) {
            bro(deleted)->color    = red;
            deleted->parent->color = black;
        } else if (bro(deleted)->color == black) {
            if ((is_left(deleted)) && (bro(deleted)->right->color == black) && (bro(deleted)->left->color == red)) {
                bro(deleted)->color       = red;
...
int main() {
    ...
    for (int N = 100; N <= N_const; N += 100) {
            Node *tree = nullptr;
        for (int i = 0; i < N; i++) {
            insert(tree, i);
        };
        int new_elem = randomizer(0, N);
        auto begin = std::chrono::steady_clock::now();
        for (unsigned cnt = 100000; cnt != 0; --cnt) {
            insert(tree, new_elem);
        }
        auto end       = std::chrono::steady_clock::now();
        auto time_span = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin);
        for (int i = 0; i < N; i++){
            if (tree == nullptr) continue;
            if ((tree->left == nullptr) & (tree->right == nullptr)) del(tree);
            else{
                if (tree->right == nullptr) del(tree->left);
                else del(tree->right);
            }
        }
