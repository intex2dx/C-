...
int main(){
    srand(999);
    std::ofstream Fout;
    Fout.open("part 3 output s2.txt");
    Fout.close();
    Fout.open("part 3 s2 swaps.txt");
    Fout.close();
    for (unsigned N = 100; N <= N_const; N += 100) {
        ...
        Fout.open("part 3 output s2.txt", std::ios_base::app);
        Fout << N << ' ' << time_span.count() << std::endl;
        Fout.close();
        Fout.open("part 3 s2 swaps.txt", std::ios_base::app);
